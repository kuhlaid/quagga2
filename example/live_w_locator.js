  ...
  var App = {
    ...
    initCameraSelection: function () {
      ...
      return Quagga.CameraAccess.enumerateVideoDevices().then(function (
        ...
        var jQuerydeviceSelection = document.getElementById("deviceSelection");
        while (jQuerydeviceSelection.firstChild) {
          jQuerydeviceSelection.removeChild(jQuerydeviceSelection.firstChild);
        }
        devices.forEach(function (device) {
          var jQueryoption = document.createElement("option");
          jQueryoption.value = device.deviceId || device.id;
          jQueryoption.appendChild(
            document.createTextNode(
              pruneText(device.label || device.deviceId || device.id)
            )
          );
          jQueryoption.selected = streamLabel === device.label;
          jQuerydeviceSelection.appendChild(jQueryoption);
        });
      });
    },
    attachListeners: function () {
      ...
      // stop camera
      jQuery(".controls").on("click", "button.stop", function (e) {
        e.preventDefault();
        Quagga.stop();
      ...
      // start camera
      jQuery(".controls").on("click", "button.start", function (e) {
        e.preventDefault();
        App.init();
        self._printCollectedResults();
      ...
      jQuery(".controls .reader-config-group").on(
        "change",
        "input, select",
        function (e) {
          e.preventDefault();
          var jQuerytarget = jQuery(e.target),
            value =
              jQuerytarget.attr("type") === "checkbox"
                ? jQuerytarget.prop("checked")
                : jQuerytarget.val(),
    ...
    detachListeners: function () {
      ...
      jQuery(".controls").off("click", "button.start");
